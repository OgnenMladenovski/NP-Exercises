...
class FlatPercentProvisionTransaction extends Transaction
{
    ...
    @Override
    public double getProvision() {
        return getAmount() * centsPerDolar / 100.0;
    }
...
class Bank {
    ...
    public boolean makeTransaction(Transaction t) {
        ...
        double cost = t.getAmount() + provision;

        if(cost > fromAccount.getBalance())
        {
            return false;
        }

        fromAccount.setBalance(fromAccount.getBalance() - cost);
    ...
    @Override
    public String toString() {
        return "Name: " + name + "\n" + "\n" + Arrays.stream(accounts).map(Account::toString).collect(Collectors.joining());
    }
