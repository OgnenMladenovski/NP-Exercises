...
public class HospitalDemo {
    public static void main(String[] args) {
        ...
        Supplier<Doctor> doctorSupplier = () -> {
            int liscence = sc.nextInt();
            String name = sc.next();
            int level = sc.nextInt();
            int patients = sc.nextInt();
            Doctor doctor = new Doctor(liscence, name, level, patients);
            return doctor;
        };
        ...
        Predicate<Doctor> filter = d -> d.getPatients() > 20;
        DoctorEvaluator highLevel = new HighExpertiseEvaluator();
        DoctorEvaluator filtered = d -> filter.test(d) && highLevel.evaluate(d);
        System.out.println("\n=== Doctors with higher number of patients and a higher level of expertise ===");
        Doctor[] passing = er.evaluate(filtered);
